...
test.describe("Phase 12 - Recruitment, Approval, Profiles & Mobile Layout", () => {
  ...
  test("P12-1 - Public Coach Application Form", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Đăng Ký Tuyển Dụng Huấn Luyện Viên" }),
    ).toBeVisible();
    ...
    await page
      .getByPlaceholder("VD: 12A Đường số 5, Phường 2, Quận Bình Thạnh, TP.HCM")
      .fill("123 Đường Sân Cầu Lông, Quận 7, TP.HCM");

    // Fill professional info
    await page.locator('input[type="number"]').fill("5");
    await page
      .getByPlaceholder("VD: Đánh đôi, Kỹ thuật nâng cao, Đào tạo trẻ em")
      .fill("Kỹ thuật đập cầu & Đánh đôi");
    await page
      .getByPlaceholder(
        "VD: Đạt huy chương Vàng giải các câu lạc bộ TPHCM 2024. Đã huấn luyện 2 học viên đạt thành tích cấp học sinh thành phố.",
      )
      .fill("Vô địch đôi nam CLB Cầu Lông Sài Gòn 2024");
    await page
      .getByPlaceholder(
        "Giới thiệu phong cách giảng dạy, tâm huyết hoặc bài học kinh nghiệm bạn muốn truyền tải tới học viên...",
      )
      .fill("Giảng dạy tận tâm, nhiệt huyết.");

    // Add certification
    await page
      .getByPlaceholder("VD: Chứng chỉ BWF Level 1, Cử nhân Sư phạm TDTT...")
      .fill("Chứng chỉ BWF Level 1");
    ...
    await expect(
      page.getByRole("heading", { name: "Nộp Hồ Sơ Thành Công!" }),
    ).toBeVisible({ timeout: 15_000 });
    await expect(
      page.getByText(/Cảm ơn bạn đã ứng tuyển vị trí/),
    ).toBeVisible();
  });
  ...
  test("P12-2 - Public Assistant Application Form", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Đăng Ký Tuyển Dụng Trợ Giảng" }),
    ).toBeVisible();
    ...
    await page
      .getByPlaceholder(
        "VD: 45/3 Đường Phạm Thế Hiển, Phường 4, Quận 8, TP.HCM",
      )
      .fill("456 Đường Sân Cầu Lông, Quận 3, TP.HCM");

    // Fill assistant professional info
    await page
      .getByPlaceholder("VD: Đại học Sư phạm TDTT TPHCM")
      .fill("Đại học Thể dục Thể thao");
    await page
      .getByPlaceholder("VD: Giáo dục Thể chất, Quản lý thể thao")
      .fill("Giáo dục Thể chất");
    await page
      .getByPlaceholder("VD: Sinh viên năm 2 / Đang đi làm")
      .fill("Năm 3");
    await page
      .getByPlaceholder(
        "VD: Có kỹ năng giao tiếp tốt với trẻ nhỏ, biết kỹ thuật sơ cứu chấn thương cơ bản, quản lý hồ sơ...",
      )
      .fill("Thị phạm động tác, hỗ trợ chuẩn bị sân bãi");
    await page
      .getByPlaceholder(
        "Mô tả số năm chơi cầu lông, trình độ cá nhân hiện tại, các hoạt động cầu lông bạn hay tham gia và tại sao bạn ứng tuyển...",
      )
      .fill("Năng động, nhiệt tình, yêu thích cầu lông.");
  ...
  test("P12-3 - Admin Approves Recruitment Applications", async ({ page }) => {
    ...
    // 1. Approve Coach
    await page
      .getByPlaceholder("Tìm theo họ tên, số điện thoại, email...")
      .fill(coachEmail);
    ...
    await expect(
      page.getByRole("heading", { name: "Phê duyệt tuyển dụng" }),
    ).toBeVisible();
    await page.locator("#temp-password").fill("TPB@123");
    await page.getByRole("button", { name: "Xác nhận duyệt" }).click();

    // Toast: Success
    await expect(page.getByText("Đã phê duyệt ứng tuyển").first()).toBeVisible({
      timeout: 15_000,
    });
    ...
    await page
      .getByRole("tab", { name: /Đã từ chối|Đã duyệt/ })
      .filter({ hasText: "Đã duyệt" })
      .click();
    await page
      .getByPlaceholder("Tìm theo họ tên, số điện thoại, email...")
      .fill(coachEmail);
  ...
  test("P12-4 - Admin Users Profile Management Verification", async ({
    page,
  }) => {
    // Login as Admin
    await loginAs(page, "admin");
    ...
    await page
      .getByPlaceholder("Tìm theo tên hoặc số điện thoại...")
      .fill(coachName);
  ...
  test("P12-5 - Mobile Layout Verification (Curved Dock & SubHeader)", async ({
    page,
  }) => {
    // Set Mobile Viewport
    ...
    await page.route("**/auth/v1/signup**", async (route) => {
      ...
      try {
        const user = await createAdminUser(
          postData.email,
          postData.password,
          postData.data || postData.options?.data || {},
        );
        const responseData = {
          user,
          session: null,
    ...
    await page
      .getByRole("button", { name: "Đăng ký tài khoản Phụ huynh" })
      .click();
    ...
    await expect(
      mobileBottomBar.locator('a[href="/parent/dashboard"]'),
    ).toBeVisible();
    await expect(
      mobileBottomBar.locator('a[href="/parent/family"]'),
    ).toBeVisible();
    ...
    await page
      .getByPlaceholder("Bé bị cận thị, hen suyễn nhẹ...")
      .fill("Bé khỏe mạnh");
    ...
    // Close payment modal
    await expect(page.getByText("Mã QR Thanh Toán Học Phí")).toBeVisible({
      timeout: 15_000,
    });
