...
export default function AdminStaffRegistrationsPage() {
  ...
  const [roleFilter, setRoleFilter] = useState<"all" | "coach" | "assistant">(
    "all",
  );
  ...
  const [tempPassword, setTempPassword] = useState("TPB@123");
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (!res.ok || !result.success || !result.user_id) {
        throw new Error(
          result.error ?? "Lỗi khi gọi chức năng tạo tài khoản nhân sự",
        );
      }
      ...
      const roleLabel =
        record.role === "coach" ? "Huấn luyện viên" : "Trợ giảng";
      ...
      toast({
        title: "Đã phê duyệt ứng tuyển",
        description: `Đã tạo tài khoản cho ${record.last_name} ${record.first_name} với mật khẩu tạm thời.`,
      });
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Đã chuyển trạng thái đơn ứng tuyển của ${record.last_name} ${record.first_name} thành từ chối.`,
      });
      ...
      return (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-sm text-gray-500 font-medium">
            Đang tải dữ liệu đơn tuyển dụng...
          </p>
        </div>
      );
    }
        ...
        <div className="hidden md:block bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mt-1">
          <Table>
            <TableHeader className="bg-gray-50/50">
              <TableRow>
                ...
                {list[0]?.status !== "pending" && (
                  <TableHead>Chi tiết duyệt</TableHead>
                )}
            ...
            <TableBody>
              {list.map((row) => {
                const RoleIcon =
                  row.role === "coach" ? Dumbbell : GraduationCap;
                        ...
                        {row.status === "approved" ? (
                          ...
                          <div className="max-w-[200px]">
                            ...
                            {row.rejection_reason && (
                              <p
                                className="text-[10px] text-gray-400 italic truncate mt-0.5"
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            setDetailDialog({ open: true, record: row })
                          }
                        ...
                        {row.status === "pending" && (
                          <>
                            <button
                              onClick={() =>
                                setApproveDialog({ open: true, record: row })
                              }
                            ...
                            <button
                              onClick={() =>
                                setRejectDialog({ open: true, record: row })
                              }
        ...
        <div className="flex md:hidden flex-col gap-3 mt-2">
          {list.map((row) => {
            ...
            return (
              <div
                key={row.id}
                className="flex flex-col border border-gray-150/70 rounded-2xl p-4 gap-3 bg-white shadow-2xs relative"
              >
                ...
                <div className="bg-gray-50/55 border border-gray-150/40 rounded-xl p-3 space-y-2 mt-1">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span
                      className={cn(
                        ...
                        row.role === "coach"
                          ? "bg-court-50 text-court-700 border border-court-200"
                  ...
                  <div className="flex justify-between items-center text-xs border-t border-gray-100 pt-2 mt-1">
                    ...
                    <span className="font-medium text-gray-750 flex items-center gap-1">
                      <Phone className="w-3 h-3 text-gray-450" /> {row.phone}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-xs border-t border-gray-100 pt-2">
                    ...
                    <span className="font-medium text-gray-750 flex items-center gap-1">
                      <Mail className="w-3 h-3 text-gray-450" /> {row.email}
                    </span>
                  </div>
                </div>
                ...
                {row.status !== "pending" && (
                  <div className="flex flex-col gap-1.5 pt-1.5 border-t border-dashed border-gray-100 text-xs">
                    {row.status === "approved" ? (
                      <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-green-100 text-green-755 border border-green-200 self-start">
                        Đã cấp tài khoản
                      </span>
                    ) : (
                      <div className="flex flex-col gap-1.5 w-full">
                        <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-red-100 text-red-750 border border-red-200 self-start">
                          Bị từ chối
                        </span>
                        {row.rejection_reason && (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900">
            Đơn ứng tuyển HLV & Trợ giảng
          </h2>
          <p className="text-sm text-gray-500 mt-0.5">
            Duyệt hồ sơ tuyển dụng và cấp tài khoản công tác
          </p>
        </div>
      </div>
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          !open && setDetailDialog({ open: false, record: null })
        }
      >
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-5 bg-gray-50 border border-gray-200 rounded-2xl p-5 my-2">
                <div className="w-20 h-20 bg-white border border-gray-200 rounded-2xl flex items-center justify-center overflow-hidden flex-shrink-0 shadow-sm">
                  {detailDialog.record.avatar_url ? (
                    <img
                      src={detailDialog.record.avatar_url}
                      alt="Avatar"
                      className="w-full h-full object-cover"
                    />
                  ) : (
              ...
              <div className="space-y-5 text-sm py-2">
                ...
                {detailDialog.record.role === "coach" ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-primary-50/20 border border-primary-100 rounded-2xl p-4">
                    <div>
                      ...
                      <p className="font-semibold text-gray-900 mt-0.5">
                        {detailDialog.record.experience_years} năm
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                      ...
                      <p className="font-semibold text-gray-900 mt-0.5">
                        {detailDialog.record.specialty ?? "Chưa cập nhật"}
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-orange-50/20 border border-orange-100 rounded-2xl p-4">
                    <div>
                      ...
                      <p className="font-semibold text-gray-900 mt-0.5">
                        {detailDialog.record.school_university ??
                          "Chưa cập nhật"}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-semibold text-gray-900 mt-0.5">
                        {detailDialog.record.major ?? "Chưa cập nhật"}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                      ...
                      <p className="font-semibold text-gray-900 mt-0.5">
                        {detailDialog.record.year_of_study ?? "Chưa cập nhật"}
                      </p>
                    </div>
                ...
                {detailDialog.record.bio && (
                  <div>
                    <h4 className="font-bold text-gray-900 text-xs border-l-2 border-primary-500 pl-2 mb-1.5">
                      GIỚI THIỆU BẢN THÂN
                    </h4>
                ...
                {/* Bằng cấp & Chứng chỉ */}
                {detailDialog.record.certifications &&
                  detailDialog.record.certifications.length > 0 && (
                    <div>
                      <h4 className="font-bold text-gray-900 text-xs border-l-2 border-primary-500 pl-2 mb-2">
                        BẰNG CẤP & CHỨNG CHỈ
                      </h4>
                      <div className="flex flex-wrap gap-2">
                        {detailDialog.record.certifications.map((cert) => (
                          <span
                            key={cert}
                            className="bg-gray-100 text-gray-800 text-xs px-3 py-1 rounded-full border border-gray-200 shadow-sm font-medium"
                          >
                            {cert}
                          </span>
                        ))}
                ...
                {detailDialog.record.status === "rejected" &&
                  detailDialog.record.rejection_reason && (
                    <div className="bg-red-50 border border-red-100 rounded-2xl p-4 flex gap-3 text-red-900 text-xs font-medium">
                      <ShieldAlert className="w-5 h-5 text-red-500 flex-shrink-0" />
                      <div>
                        <p className="font-bold">
                          Lý do từ chối hồ sơ ứng tuyển:
                        </p>
                        <p className="mt-1 leading-relaxed text-red-750 font-normal">
                          {detailDialog.record.rejection_reason}
                        </p>
                      </div>
                    </div>
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          !open && setApproveDialog({ open: false, record: null })
        }
      >
        <DialogContent className="sm:max-w-md rounded-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Hệ thống sẽ tự động tạo tài khoản đăng nhập trên Supabase Auth và
              phân vai trò tương ứng trong cơ sở dữ liệu.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-3 text-sm">
            {approveDialog.record && (
              <div className="p-3 bg-green-50 border border-green-200 rounded-2xl font-medium text-green-900">
                Phê duyệt ứng viên: **{approveDialog.record.last_name}{" "}
                {approveDialog.record.first_name}** làm **
                {approveDialog.record.role === "coach"
                  ? "Huấn luyện viên"
                  : "Trợ giảng"}
                **.
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[10px] text-gray-400">
                Ứng viên sẽ sử dụng Email ứng tuyển và mật khẩu này để đăng nhập
                lần đầu.
              </p>
            </div>
          </div>
          <DialogFooter>
            ...
            >
              {isProcessing ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin mr-1.5" /> Đang
                  duyệt...
                </>
              ) : (
                "Xác nhận duyệt"
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          !open && setRejectDialog({ open: false, record: null })
        }
      >
        <DialogContent className="sm:max-w-md rounded-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Chuyển hồ sơ sang trạng thái từ chối. Bạn có thể nhập lý do để lưu
              vết hoặc phản hồi.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-3 text-sm">
            {rejectDialog.record && (
              <div className="p-3 bg-red-50 border border-red-200 rounded-2xl font-medium text-red-900">
                Từ chối ứng viên: **{rejectDialog.record.last_name}{" "}
                {rejectDialog.record.first_name}** ứng tuyển làm **
                {rejectDialog.record.role === "coach"
                  ? "Huấn luyện viên"
                  : "Trợ giảng"}
                **.
              </div>
          ...
          <DialogFooter>
            ...
            >
              {isProcessing ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin mr-1.5" /> Đang cập
                  nhật...
                </>
              ) : (
                "Xác nhận từ chối"
